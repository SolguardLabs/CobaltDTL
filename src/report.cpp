...
void writePolicy(JsonWriter& writer, const VaultPolicy& policy) {
    ...
    writer.key("reserveHaircutBps");
    writer.value(policy.reserveHaircutBps);
    writer.key("redemptionShockBps");
    writer.value(policy.redemptionShockBps);
    writer.key("operationalBufferBps");
    writer.value(policy.operationalBufferBps);
    writer.key("minimumCapitalCoverageBps");
    writer.value(policy.minimumCapitalCoverageBps);
    writer.key("minimumLiquidityCoverageBps");
    writer.value(policy.minimumLiquidityCoverageBps);
    writer.key("maturityEpochs");
    writer.value(policy.maturityEpochs);
...
void writeAnalyticsVaults(JsonWriter& writer, const std::vector<VaultHealth>& vaults) {
    ...
    for (const VaultHealth& health : vaults) {
        ...
        writer.key("capital");
        writer.beginObject();
        writer.key("liability");
        writer.value(health.capitalLiability);
        writer.key("effectiveReserve");
        writer.value(health.effectiveReserve);
        writer.key("stressedOutflows");
        writer.value(health.stressedOutflows);
        writer.key("requiredCapital");
        writer.value(health.requiredCapital);
        writer.key("deficit");
        writer.value(health.capitalDeficit);
        writer.key("coverageBps");
        writer.value(health.capitalCoverageBps);
        writer.key("liquidityBps");
        writer.value(health.liquidityCoverageBps);
        writer.key("compliant");
        writer.value(health.capitalCompliant);
        writer.endObject();
        writer.key("notes");
        writer.beginArray();
